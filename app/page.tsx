...
export default function HomePage() {
  ...
  return (
    <div className="container max-w-md mx-auto px-4 py-8 relative">
      ...
      <Butterfly size={18} color="#F472B6" top="10%" right="5%" delay={2} duration={12} />
      <Butterfly size={15} color="#60A5FA" top="30%" left="5%" delay={5} duration={10} />
      <Butterfly size={20} color="#34D399" top="60%" right="10%" delay={8} duration={15} />
        ...
        <AnimatePresence mode="wait">
          {plants.length === 0 ? (
            ...
            >
              ...
              <p className="text-muted-foreground mb-4">Start adding your plant friends to track their care</p>
              <Link href="/add">
            ...
            <motion.div key="plants" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              {/* Plants count */}
              <div className="flex items-center justify-between mb-3">
                ...
                {searchQuery && filteredPlants.length !== plants.length && (
                  <Button variant="ghost" size="sm" className="text-xs h-7 px-2" onClick={() => setSearchQuery("")}>
                    Clear filter
                  </Button>
                )}
              </div>
